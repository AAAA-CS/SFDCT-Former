...
def select_small_cubic(data_size, data_indices, whole_data, patch_length, padded_data, dimension):
    small_cubic_data = np.zeros((data_size, patch_length, patch_length, dimension),dtype=np.float16)
...
def generate_iter(train_indices, test_indices, whole_data,PATCH_SIZE, batch_size, gt):
    ...
    # padded_data_pca = np.lib.pad(data_pca, ((PATCH_SIZE, PATCH_SIZE), (PATCH_SIZE, PATCH_SIZE),
    #                                       (0, 0)), 'constant', constant_values=0)
    ...
    TRAIN_SIZE_pca = len(train_indices)
    VAL_SIZE_pca = len(train_indices)
    TEST_SIZE_pca = len(test_indices)
    TOTAL_SIZE_pca = len(total_indices)
    # INPUT_DIMENSION_pca = data_pca.shape[2]

    print("输入维度",INPUT_DIMENSION)
    # print("PCA后的维度",INPUT_DIMENSION_pca )
    ...
    train_data = select_small_cubic(TRAIN_SIZE, train_indices, whole_data,
                                    PATCH_SIZE, padded_data, INPUT_DIMENSION)
    #PCA后的训练数据
    # train_data_pca = select_small_cubic(TRAIN_SIZE_pca, train_indices, data_pca,
    #                                 PATCH_SIZE, padded_data_pca, INPUT_DIMENSION_pca)

    # print(train_data.shape)
    test_data = select_small_cubic(TEST_SIZE, test_indices, whole_data,
                                   PATCH_SIZE, padded_data, INPUT_DIMENSION)
    #
    # test_data_pca = select_small_cubic(TEST_SIZE_pca, test_indices, data_pca,
    #                                PATCH_SIZE, padded_data_pca, INPUT_DIMENSION_pca)
    ...
    x_train = train_data.reshape(train_data.shape[0], train_data.shape[1],
                                 train_data.shape[2], INPUT_DIMENSION)
    x_test_all = test_data.reshape(test_data.shape[0], test_data.shape[1],
                                   test_data.shape[2], INPUT_DIMENSION)
    # x_train_pca = train_data_pca.reshape(train_data_pca.shape[0], train_data_pca.shape[1],
    #                              train_data_pca.shape[2], INPUT_DIMENSION_pca)
    # x_test_all_pca = test_data_pca.reshape(test_data_pca.shape[0], test_data_pca.shape[1],
    #                                test_data_pca.shape[2], INPUT_DIMENSION_pca)

    x_val = x_test_all[-VAL_SIZE:]
    # x_val_pca = x_test_all_pca[-VAL_SIZE_pca:]
    y_val = y_test[-VAL_SIZE:]

    x_test = x_test_all[:-VAL_SIZE]
    # x_test_pca = x_test_all_pca[:-VAL_SIZE_pca]
    y_test = y_test[:-VAL_SIZE]

    x1_tensor_train = torch.from_numpy(x_train).type(torch.FloatTensor).unsqueeze(1)
    # x1_tensor_train_pca = torch.from_numpy(x_train_pca).type(torch.FloatTensor).unsqueeze(1)
    ...
    # x1_tensor_valida_pca = torch.from_numpy(x_val_pca).type(torch.FloatTensor).unsqueeze(1)
    ...
    # x1_tensor_test_pca = torch.from_numpy(x_test_pca).type(torch.FloatTensor).unsqueeze(1)
